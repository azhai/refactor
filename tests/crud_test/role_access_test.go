...
func Test01InsertAccess(t *testing.T) {
	...
	// 超管可以访问所有菜单
	m, err = contrib.AddAccess("superuser", "menu", contrib.ACCESS_ALL, "*")
	assert.NoError(t, err)
	assert.Equal(t, m.Id, 1)
	// 基本用户
	m, err = contrib.AddAccess("visitor", "menu", contrib.ACCESS_VIEW, "/dashboard")
	assert.NoError(t, err)
	assert.Equal(t, m.Id, 2)
	m, err = contrib.AddAccess("visitor", "menu", contrib.ACCESS_VIEW, "/error/404")
	assert.NoError(t, err)
